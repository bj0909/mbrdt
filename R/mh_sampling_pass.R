#' Metropolis-Hastings Method for sampling the posterior distribution given Passing the RDT
#'
#' @description This function uses the Metropolis-Hastings algorithm to generate a sample of
#' failure probabilities given passing the RDT.
#'
...
#' @return A vector containing the Markov Chain sample of failure probabilities for the posterior given passing the RDT.
#'
#' @examples
#' samples_pass <- mh_sampling_pass(sampleSize = 100,
#' startVal = 0.2,
#' sd = 0.1,
#' n = 100,
#' c = 10,
...
mh_sampling_pass <- function(sampleSize, startVal, sd, n, c,
  ...
  for(i in 2:sampleSize){
    ...
    if(!(proposed>0 & proposed<1)){
      ratio <- 0
    } else if (targetPass(current, n, c, betaPriorParam1, betaPriorParam2)==0) {
      ratio <- 1
    } else {
      ratio <- min(1, exp(log(targetPass(proposed, n, c, betaPriorParam1, betaPriorParam2))-
                            log(targetPass(current, n, c, betaPriorParam1, betaPriorParam2))))
    }
